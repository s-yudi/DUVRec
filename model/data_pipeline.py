...
def generate_batch_train(path, batch_size, device, n_neg=5):
    ...
    while True:
        ...
        while(len_batch < batch_size):
            ...
            adj.append(generate_adj(sample[:hst_size], attr_itm_list, stp_train))
...
def generate_batch_test(path, device):
    ...
    for test, attr, test_cdd in zip(test_all, attr_all, test_cdd_all):
        ...
        _adj    = generate_adj(sample[:hst_size], attr_itm_list, stp_test)
...
def generate_adj(hst, attr_item_list, stp):
    '''
    input:
        hst: [itm_id]
        stp: [stp]
    ...
    tmp = torch.abs(stp[:dim_hst].view(-1, 1) - stp[:dim_hst].view(1, -1)) + 1e-6
    tmp = torch.log(1/tmp)
    adj[:dim_hst, :dim_hst] = torch.where(tmp>1, tmp, torch.ones((dim_hst,dim_hst)))
    adj[:dim_hst, :dim_hst] -= torch.diag(torch.diag(adj[:dim_hst, :dim_hst]))
...
if __name__=='__main__':
    ...
    # u, c = next(generate_batch_train('./data/movielens/', batch_size=5, device=device))
